...
# 판매현황
cur.execute("SELECT COUNT(*) FROM ecount_sales")
sales_count = cur.fetchone()[0]
print(f"판매현황 (ecount_sales): {sales_count:,}건")

cur.execute("SELECT MIN(date), MAX(date) FROM ecount_sales")
row = cur.fetchone()
if row[0]:
    print(f"  - 기간: {row[0]} ~ {row[1]}")

# 구매현황
cur.execute("SELECT COUNT(*) FROM ecount_purchase")
purchase_count = cur.fetchone()[0]
print(f"\n구매현황 (ecount_purchase): {purchase_count:,}건")

cur.execute("SELECT MIN(date), MAX(date) FROM ecount_purchase")
row = cur.fetchone()
if row[0]:
    print(f"  - 기간: {row[0]} ~ {row[1]}")

# 생산입고현황
cur.execute("SELECT COUNT(*) FROM ecount_production")
production_count = cur.fetchone()[0]
print(f"\n생산입고현황 (ecount_production): {production_count:,}건")

cur.execute("SELECT MIN(date), MAX(date) FROM ecount_production")
row = cur.fetchone()
if row[0]:
    print(f"  - 기간: {row[0]} ~ {row[1]}")
